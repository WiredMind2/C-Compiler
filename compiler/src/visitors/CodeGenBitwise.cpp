...
antlrcpp::Any visitUnaryNot(CodeGenVisitor* visitor, ifccParser::UnaryNotContext* ctx) {
    ...
    string zero_tmp = bb->create_new_tempvar(value.type);
    bb->add_IRInstr(new LdConstInstr(bb, Reg::W0, value.type, (int64_t)0));
    bb->add_IRInstr(new StoreStackInstr(bb, zero_tmp, Reg::W0, value.type));
    StackParam zero(zero_tmp, value.type);
...
antlrcpp::Any visitShiftExprRef(CodeGenVisitor* visitor, ifccParser::ShiftExprRefContext* ctx) { return visitor->visit(ctx->additive()); }
