...
antlrcpp::Any visitVar_decl_with_init(CodeGenVisitor* visitor, ifccParser::Var_decl_with_initContext *ctx)
{
    ...
    if (type == IRType::FLOAT64 && src.type != IRType::FLOAT64) {
        // int/char -> double: load as int, convert to float
        bb->add_IRInstr(new LoadStackInstr(bb, Reg::W0, src.name, src.type));
        bb->add_IRInstr(new IToFInstr(bb, Reg::W0, Reg::W0));
        bb->add_IRInstr(new StoreStackInstr(bb, var, Reg::W0, IRType::FLOAT64));
    } else if (src.type == IRType::FLOAT64 && type != IRType::FLOAT64) {
        // double -> int/char: load as double, convert to int
        bb->add_IRInstr(new LoadStackInstr(bb, Reg::W0, src.name, IRType::FLOAT64));
        bb->add_IRInstr(new FToIInstr(bb, Reg::W0, Reg::W0));
