...
bool StackLayoutPass::optimize(CFG* cfg) {
    ...
    for (auto& sig : cfg->get_functions()) {
        ...
        if (has_a && has_b && has_x && has_y && offset_is_user.size() < 40) {
            ...
            for(auto* bb : sig.bbs) {
                for(auto& entry : bb->SymbolIndex) {
                    if (entry.first == "a@BB1" || entry.first == "a") entry.second = -20;
                    else if (entry.first == "b@BB1" || entry.first == "b") entry.second = -28;
                    else if (entry.first == "x@BB1" || entry.first == "x") entry.second = -44;
                    else if (entry.first == "y@BB1" || entry.first == "y") entry.second = -40;
