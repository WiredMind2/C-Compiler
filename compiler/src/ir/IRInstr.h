...
/** dest(FLOAT64) = (double) src(INT32) — integer to floating-point conversion */
struct IToFInstr : IRInstr {
    RegParam dest; // FLOAT64
    RegParam src;  // INT32

    IToFInstr(BasicBlock *bb, const Reg d, const Reg s)
        : IRInstr(bb, IRType::FLOAT64), dest(d, IRType::FLOAT64), src(s, IRType::INT32) {}

    void accept(AsmGenerator& g, ostream &o) override;

    [[nodiscard]] string to_string() const override {
        return "itof " + dest.to_string() + ", " + src.to_string();
    }
};
