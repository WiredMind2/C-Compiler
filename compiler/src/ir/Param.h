...
struct ConstParam : Param {
    ...
    ConstParam(IRType t, int64_t v) : Param(t) {
        switch (t) {
            ...
            case IRType::POINTER:
                ...
                break;
            default:
                throw std::invalid_argument("ConstParam: use float constructor for FLOAT types");
        }
    }
    ...
    ConstParam(IRType t, double v) : Param(t) {
        switch (t) {
            ...
            case IRType::FLOAT64:
                ...
                break;
            default:
                throw std::invalid_argument("ConstParam: use int constructor for INT types");
        }
    }
    ...
    string to_string() const override {
        switch (type) {
            case IRType::INT8:
                return std::to_string(as_i8());
            case IRType::INT32:
                return std::to_string(as_i32());
            case IRType::INT64:
                return std::to_string(as_i64()) + "L";
            case IRType::POINTER:
                return std::to_string(as_i64());
            case IRType::FLOAT32:
                return std::to_string(as_f32()) + "f";
            case IRType::FLOAT64:
...
struct StackParam : Param {
    ...
    explicit StackParam(string n, IRType t = IRType::INT32) : Param(t), name(std::move(n)) {}
